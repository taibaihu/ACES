...
- 用户说：「分析一下 BTC 短线」「给我个 ETH 5m 交易信号」「用 LLM 看看现在能不能开多」

## 核心能力

1. **多周期特征工程**：5m（主）+ 15m（辅助）+ 4h / 1d（宏观过滤），计算 EMA9/21/50/200、RSI、MACD、布林带、ATR、KDJ、量比
...
# 1) 模拟演示（无需任何 API）
python scripts/analyze.py --mock --symbol BTC-USDT --timeframe 5m
